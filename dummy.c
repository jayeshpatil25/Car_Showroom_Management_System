...
void load_car_data(const char *filename, car **head) {
    ...
    if (!file) {
        printf("Error opening car data file.\n");
        return;
    }
...
void load_salesperson_data(const char *filename, salesperson **head) {
    FILE *file = fopen(filename, "r");
    if (!file) {
        printf("Error opening salesperson data file.\n");
        return;
    }
...
void load_model_sales_data(const char *filename, ModelSales **hashmap) {
    FILE *file = fopen(filename, "r");
    if (!file) {
        printf("Error opening model sales data file.\n");
        return;
    }
    ...
    while (fscanf(file, "%s, %d\n", model_name, &total_sold) == 2) {
        ...
        HASH_ADD_STR(*hashmap, model_name, new_model_sales);
    }

    fclose(file);
}

// Function to load sales history data into the array of SalesHistory
void load_sales_history_data(const char *filename, SalesHistory sales_history[], int *num_models) {
    FILE *file = fopen(filename, "r");
    if (!file) {
        printf("Error opening sales history data file.\n");
        return;
    }

    char model_name[MAX_STRING_LEN];
    int sales_data[MONTHS];

    while (fscanf(file, "%s, ", model_name) == 1) {
        SalesHistory new_history;
        strcpy(new_history.model_name, model_name);

        // Read the sales data for the last 12 months
        for (int i = 0; i < MONTHS; i++) {
            fscanf(file, "%d ", &sales_data[i]);
        }

        // Copy sales data to the structure
        memcpy(new_history.sales, sales_data, sizeof(sales_data));

        // Store this SalesHistory in the array
        sales_history[*num_models] = new_history;
        (*num_models)++;  // Increase the model count
    }
